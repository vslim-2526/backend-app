...
3. Update version in `package.json` and **/health endpoint**, eg. **1.0.1**.
...
1. Update version in `package.json`, eg. **1.0.1-rc.3**.
2. Commit that, then create lightweight tag for the code with `git tag <prerelease-version>`, eg. **git tag 1.0.1-rc.3**.
3. From CLI, run `docker build -t dongphong543/vslim-backend-app:<prerelease-version> .` (notice the last dot). This will create 1 image: **dongphong543/vslim-backend-app:\<prerelease-version\>**.
4. Push this image to our Docker Hub repository.
5. Once done, push branch `prerelease/<future-version>-rc.<number>` to Github with `--tags` option, and _leave it there_.
